...
class Security
{
	...
	public static function output($buffer, $type = 'html', $cache_days = null, $compress = true) {
		...
		if (self::$headers_cache)  @header('Content-Length: ' . strlen($buffer)); // For cache header
	...
	public static function compressCSS($buffer) {
		...
		return preg_replace(array('#\/\*[\s\S]*?\*\/#u', '/\s+/u'), array('', ' '), str_replace(array("\n", "\r", "\t"), '', $buffer));
	}
	...
	public static function compressJS($buffer) {
		...
		return str_replace(array("\n", "\r", "\t"), '', preg_replace(array('#\/\*[\s\S]*?\*\/|([^:]|^)\/\/.*$#um', '/\s+/u'), array('', ' '), $buffer));
	}
